...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadTasks() {
      setLoading(true)
      const { data, error } = await client.from('userinfo').select()
  ...
  async function createTask(e: React.FormEvent<HTMLFormElement>) {
    ...
    await client.from('userinfo').insert({
      name,
    })
  ...
  return (
    <div>
      ...
      {!loading && userinfo.length > 0 && userinfo.map((task: any) => <p key={task.id}>{task.name}</p>)}

      {!loading && userinfo.length === 0 && <p>No tasks found</p>}
